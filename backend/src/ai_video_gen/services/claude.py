...
ユーザーが提供したドキュメント（箇条書き、メモ、原稿など）を、
ナレーション形式の動画脚本に変換してください。

入力されたドキュメントの内容を活かしつつ、以下の形式で自然な話し言葉のナレーションに変換してください。
...
- 原稿の段落や話題の切り替わりを検出してセクションに分割する
- 段落が長い場合はさらにトピック単位に分割する
- 元の原稿の文章・表現はできるだけ維持する（勝手に書き換えない）

注意:
- 入力されたドキュメントの内容を忠実に反映
- narrationは話し言葉で自然に（「〜ですね」「〜しましょう」など）
